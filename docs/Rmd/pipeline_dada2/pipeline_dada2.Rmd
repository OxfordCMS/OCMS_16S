...
dat <- dat[,2:ncol(dat)]
new.colnames <- factor(sample.map[gsub("\\.", "-", colnames(dat)),]$reportId, levels=ids)

# need to catch if the colnames are not usual
if (is.na(new.colnames[1])){
  new.colnames <- factor(levels(new.colnames), levels=levels(new.colnames))
}
