...
```{r taxonomy, fig.cap='Taxonomic assignments of ASVs', fig.height=10, fig.width=10,  echo=FALSE, message=FALSE}

library(data.table)
dat <- data.frame(fread("abundance.dir/taxa_abundances.tsv", header=TRUE, stringsAsFactors=F, sep="\t"))
rownames(dat) <- dat$sequence
dat <- dat[,2:ncol(dat)]
...
## Number of ASVs assigned at different taxonomic levels

It is useful to assess the proportion of ASVs that could be successfully assigned to taxa. We would expect (in most cases) that the majority of ASVs woild be assigned at high taxonomic ranks (e.g. phylum) and fewer at lower taxonomic ranks (e.g. species).

```{r nasvs.assigned, fig.height=5, fig.width=7, echo=FALSE, message=FALSE}
...
p7 <- ggplot(nasvs.assigned, aes(x=level, y=percent.assigned, fill=level))
p8 <- p7 + geom_bar(stat="identity")
p9 <- p8 + theme_bw() 
p10 <- p9 + ylab("% assigned") + xlab("Taxonomic levels")
p10 + scale_fill_manual(values=c("red3", "blue3", "green3", "orange3", "purple3", "grey", "black"))
