...
```{r taxonomy, fig.cap='Taxonomic assignments of ASVs', fig.height=5, fig.width=8,  echo=FALSE, message=FALSE}
...
dp <- melt(aggregate(dat, by=list(p), sum))
dc <- melt(aggregate(dat, by=list(c), sum))
do <- melt(aggregate(dat, by=list(o), sum))
df <- melt(aggregate(dat, by=list(f), sum))
dg <- melt(aggregate(dat, by=list(g), sum))
